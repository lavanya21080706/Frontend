...
function EditpopUp({ editData, onClose, cb, _id }) {
  ...
  useEffect(() => {
    if (editData) {
      setOldData({
        ...
        cb: Array.isArray(editData.cb) ? [...editData.cb] : [],
      });
  ...
  useEffect(() => {
    if (editData && Array.isArray(oldData.cb)) {
      const cbValues = oldData.cb;


      const newInputFields = oldData.checklist.map((item, index) => {
        const checked = cbValues.includes(item.value.trim());
  ...
  }, [editData, oldData.cb, oldData.checklist]);
  ...
  const handleSavePopup = async () => {
    try {
      ...
      console.log(`console saved data is ${newSavedData}`)
              ...
              {inputFields.map((field) => (
                ...
                >
                  <input
                    ...
                    {...(cb?.includes(field.value.trim()) && { checked: true })}
